...
import { getLibraryItems, publishContent, getConnectedPlatforms, deleteContentByBatch, deleteContentById, updateContentStatus, type LibraryContentItem, type ConnectedPlatform } from "@/lib/content-store";
...
const statusStyles: Record<string, { bg: string; text: string }> = {
  ...
  published: { bg: "bg-blue-50 dark:bg-blue-950/20", text: "text-blue-600 dark:text-blue-400" },
  processing: { bg: "bg-yellow-50 dark:bg-yellow-950/20", text: "text-yellow-600 dark:text-yellow-400" },
  draft: { bg: "bg-amber-50 dark:bg-amber-950/20", text: "text-amber-600 dark:text-amber-400" },
  archived: { bg: "bg-neutral-100 dark:bg-neutral-800/30", text: "text-neutral-400 dark:text-neutral-500" },
};
...
export default function ContentLibraryPage() {
  ...
  async function handlePublish(genId: string, platform: string) {
    const connection = connectedPlatforms.find((cp) => cp.platform === platform && cp.connected);
    if (!connection) {
      alert(`Connect ${platform} first via the Connect page to publish directly.`);
      return;
    }
  ...
  async function handleDeleteBatch(topicId: string, topicTitle: string) {
    if (!window.confirm(`Delete all scripts for "${topicTitle}"?`)) return;
  ...
  async function handleDeleteGeneration(genId: string, topicId: string) {
  ...
  const filteredItems = items.filter((item) => {
    const matchesSearch =
      ...
      item.generations.some((g) => g.content.toLowerCase().includes(searchQuery.toLowerCase()));
    const matchesFilter =
      filterStatus === "all" ||
  ...
  const totalGenerations = filteredItems.reduce((acc, item) => acc + item.generations.length, 0);
    ...
    return (
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-background dark:bg-[#121214] rounded-xl border border-foreground/5 dark:border-white/5 p-5">
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-sans text-2xl font-bold tracking-tight text-[var(--sp-fg)]">Content Library</h1>
          <p className="font-mono text-sm text-[var(--sp-fg-light)] mt-1">
            {filteredItems.length} {filteredItems.length === 1 ? "topic" : "topics"} · {totalGenerations} {totalGenerations === 1 ? "script" : "scripts"}
          </p>
        </div>
      </div>
      ...
      {filteredItems.length === 0 && (
        <div className="bg-background dark:bg-[#121214] rounded-xl border border-foreground/5 dark:border-white/5 p-16 text-center">
          ...
          <h3 className="text-sm font-semibold text-[var(--sp-fg)] mb-1">
            {searchQuery || filterStatus !== "all" ? "No matching content" : "Your library is empty"}
          </h3>
              ...
              <div key={topic.id}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      {topic.platforms.map((p) => (
                        ...
                        >
                          {platformIcon[p] || null}
                          <span className="capitalize">{platformLabel[p] || p}</span>
                        </span>
                      ))}
                      <span className="text-xs text-[var(--sp-fg-light)]">
                        · {topic.outputCount} {topic.outputCount === 1 ? "script" : "scripts"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); handleDeleteBatch(topic.id, topic.title); }}
                ...
                {isTopicExpanded && (
                  <div className="border-t border-foreground/5 dark:border-white/5 bg-black/[0.02] dark:bg-white/[0.02]">
                    {topic.generations.map((gen) => {
                      const style = statusStyles[gen.status] || statusStyles.draft;
                      const isGenExpanded = expandedGenId === gen.id;
                      const isPublishing = publishingGenId === gen.id;
                      const isPublished = gen.status === "published";

                      return (
                        <div key={gen.id} className="border-t border-foreground/5 dark:border-white/5 first:border-t-0">
                          <div
                            ...
                            onClick={() => setExpandedGenId(isGenExpanded ? null : gen.id)}
                          >
                            {/* Platform icon */}
                            <div className="w-8 h-8 rounded-lg bg-[var(--sp-bg)] flex items-center justify-center text-[var(--sp-fg-light)]">
                              {platformIcon[gen.platform] || <FileText className="w-3.5 h-3.5" />}
                            </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <div className="relative">
                                <button
                                  onClick={(e) => { e.stopPropagation(); setStatusDropdownId(statusDropdownId === gen.id ? null : gen.id); }}
                                ...
                                >
                                  {updatingStatusId === gen.id ? "..." : gen.status}
                                </button>
                                {statusDropdownId === gen.id && (
                                  <div className="absolute right-0 top-full mt-1 z-20 bg-background dark:bg-[#1a1a1e] border border-foreground/10 dark:border-white/10 rounded-lg shadow-lg py-1 min-w-[120px]">
                                    {["ready", "draft", "published", "archived"].map((s) => {
                                      const sStyle = statusStyles[s] || statusStyles.draft;
                                      return (
                                        <button
                                          key={s}
                                          onClick={(e) => { e.stopPropagation(); handleStatusChange(gen.id, s); }}
                                        ...
                                        >
                                          <span className={`w-1.5 h-1.5 rounded-full ${sStyle.text} bg-current`} />
                                          {s}
                                        </button>
                                      );
                                    })}
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); handleDeleteGeneration(gen.id, topic.id); }}
                          ...
                          {isGenExpanded && (
                            <div className="pl-14 pr-5 pb-4 border-t border-foreground/5 dark:border-white/5">
                              <div className="pt-3">
                                <pre className="text-xs whitespace-pre-wrap font-sans leading-relaxed text-[var(--sp-fg-mid)] max-h-48 overflow-y-auto mb-3">
                                  {gen.content}
                                </pre>
                                <div className="flex gap-2">
                                  <button
                                    onClick={(e) => { e.stopPropagation(); handleCopy(gen.content, gen.id); }}
                                  ...
                                  >
                                    {copiedGenId === gen.id ? <Check className="w-3 h-3 text-[var(--sp-green)]" /> : <Copy className="w-3 h-3" />}
                                    {copiedGenId === gen.id ? "Copied" : "Copy"}
                                  </button>
                                  {!isPublished && (
                                    <button
                                      onClick={(e) => { e.stopPropagation(); handlePublish(gen.id, gen.platform); }}
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    {topic.platforms.map((p) => (
                      <span key={p} className="w-6 h-6 rounded-md bg-[var(--sp-bg)] flex items-center justify-center text-[var(--sp-fg-light)]">
                        {platformIcon[p] || null}
                      </span>
                    ))}
                  </div>
                ...
                <h3 className="text-sm font-semibold mb-2 line-clamp-2">{topic.title}</h3>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-[var(--sp-fg-light)]">
                    {topic.outputCount} {topic.outputCount === 1 ? "script" : "scripts"}
                  </span>
                  <span className="text-xs text-[var(--sp-fg-light)]">{formatDate(topic.createdAt)}</span>
                </div>
                ...
                {isTopicExpanded && (
                  <div className="mt-3 pt-3 border-t border-foreground/5 dark:border-white/5 space-y-2" onClick={(e) => e.stopPropagation()}>
                    {topic.generations.map((gen) => {
                      const style = statusStyles[gen.status] || statusStyles.draft;
                      const isGenExpanded = expandedGenId === gen.id;
                      const isPublishing = publishingGenId === gen.id;
                      ...
                      return (
                        <div key={gen.id} className="rounded-lg border border-foreground/5 dark:border-white/5 overflow-hidden">
                          <div
                          ...
                          >
                            <span className="text-[var(--sp-fg-light)]">
                              {platformIcon[gen.platform] || <FileText className="w-3 h-3" />}
                            </span>
                            ...
                            <button
                              ...
                              title="Delete script"
                            >
                              {deletingId === gen.id ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <Trash2 className="w-2.5 h-2.5" />}
                            </button>
                          </div>

                          {isGenExpanded && (
                            <div className="px-3 pb-2 border-t border-foreground/5 dark:border-white/5">
                              <pre className="text-xs whitespace-pre-wrap font-sans leading-relaxed text-[var(--sp-fg-mid)] max-h-32 overflow-y-auto mt-2 mb-2">
                                {gen.content}
                              </pre>
                              <div className="flex gap-2">
                                <button
